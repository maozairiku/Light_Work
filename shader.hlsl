...
//=============================================================================
// ピクセルシェーダ(player view color control)
//=============================================================================
	...
	else
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			if (Light.Flags[i].y == 1)
			{
				...
				{
					lightDir = normalize(inWorldPos.xyz - Light.Position[i].xyz);				
					light = dot(lightDir, inNormal.xyz);
					light *= -1;
					...
					float angle = dot(lightDir, normalize(Light.Direction[i].xyz));		
					angle = abs(acos(angle));	
					att = saturate((Light.Angle[i].x - angle) / Light.Angle[i].x);
					light *= att;
